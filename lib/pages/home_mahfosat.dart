...
class HomeMahfozat extends StatefulWidget {
  final Map  hadith;
  final int index;
  const HomeMahfozat({super.key, required this.hadith,required this.index});
...
class HomeHadithState extends State<HomeMahfozat> {
    ...
    return BlocConsumer<AppCubit, AppStates>(builder: (context, state) {
      return Scaffold(key: key,
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            //          Share text
            Share.share(AppCubit
                .get(context)
                .txt!, subject: widget.hadith['nameHadith']);
          },
          tooltip: "Centre FAB",
          elevation: 4.0,
          child: Container(
                      ...
                      Padding(
                        ...
                        child: IconButton(onPressed: () {
                         ...
                         dismissDirection: DismissDirection.startToEnd,))  ;*/
                          ...
                          Icons.bookmarks_rounded, size: 40,color: Colors.amber,) : const Icon(
                          Icons.bookmarks_outlined, size: 40, )),
                      ),
                  ...
                  if(bol)
                    Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          //AppCubit.get(context). updateTabSelection
                          customRowItem(context, widget.hadith['textHadith'], 0,
                              Icons.book, () {
                                AppCubit.get(context).updateTabSelection(
                                    0, widget.hadith['textHadith']);
                              }),
                          customRowItem(
                              context, widget.hadith['translateNarrator'], 1,
                              Icons.library_books, () {
                            AppCubit.get(context).updateTabSelection(
                                1, widget.hadith['translateNarrator']);
                          }),
                          customRowItem(
                              context, widget.hadith['explanationHadith'] ?? '',
                              2, Icons.collections_bookmark, () {
                            AppCubit.get(context).updateTabSelection(
                                2, widget.hadith['explanationHadith']);
                          }),
                          customRowItem(
                              context, widget, 3, Icons.volume_up, () {
                            Navigator.push(context, MaterialPageRoute(
                                builder: (context) =>
                                    RunAudio(index: widget.index)));
                          }),
                        ],
                      ),
                    ),
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.end,
                        ...
                        children: [
                          ...
                          if (widget.hadith != null)
                            AppCubit.get(context).convertHadith(
                                context, AppCubit
                                .get(context)
                                .txt ?? widget.hadith['textHadith']) else
